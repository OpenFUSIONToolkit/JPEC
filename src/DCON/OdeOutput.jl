...
function ode_output_monitor(ctrl::DconControl, equil::Equilibrium.PlasmaEquilibrium, intr::DconInternal, odet::OdeState, outp::DconOutput)
    ...
    q, logpsi, crit = ode_output_get_crit(odet.psifac, odet.u, intr.mpert, intr.npert, equil.sq)
    ...
    if crit * odet.crit_prev < 0 # if crit changes sign, zero crossing has occurred
        ...
        q_med, logpsi_med, crit_med = ode_output_get_crit(psi_med, u_med, intr.mpert, intr.npert, equil.sq)
        ...
        if (crit_med - crit) * (crit_med - odet.crit_prev) < 0 &&
           abs(crit_med) < 0.5 * min(abs(crit), abs(odet.crit_prev))
            ...
            if outp.write_crit_out
                ...
                write_output(outp, :crit_out, @sprintf("%11.3e%11.3e%11.3e%11.3e", psi_med, dpsi, q_med, crit_med))
            end
            odet.nzero += 1
        end
    ...
    # Write new crit
    if outp.write_crit_out
        write_output(outp, :crit_out, @sprintf("%11.3e%11.3e%11.3e%11.3e", odet.psifac, dpsi, odet.q, crit))
    end
...
"""
    ode_output_get_crit(psi, u, mpert, npert, sq) -> (q, logpsi, crit)
...
"""
function ode_output_get_crit(psi::Float64, u::Array{ComplexF64,3}, mpert::Int, npert::Int, sq::Spl.CubicSpline)

    # TODO: not sure why 1:mpert is needed here for indexing (something to do with res_flag = true increasing msol?)
    # Will need to figure out later how to integrate with multi-n, for now just taking the whole thing
    # Compute inverse plasma response matrix
    wp = adjoint(u[:, 1:mpert * npert, 1])         # adjoint = conjugate transpose
    temp = adjoint(u[:, 1:mpert * npert, 2])
